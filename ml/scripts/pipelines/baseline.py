    ...
    for i, row in tqdm(taxonomy.iterrows()):
        if isinstance(row["Уровень 1 (iab)"], str):
            tags[row["Уровень 1 (iab)"]] = (
                model.encode(row["Уровень 1 (iab)"], convert_to_tensor=True).cpu().numpy()
            )  # .tolist()
        if isinstance(row["Уровень 2 (iab)"], str):
            tags[row["Уровень 1 (iab)"] + ": " + row["Уровень 2 (iab)"]] = (
                model.encode(row["Уровень 1 (iab)"] + ": " + row["Уровень 2 (iab)"], convert_to_tensor=True)
                .cpu()
                .numpy()
            )  # .tolist()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--submission_file', default=f"data/submits/baselinesample_submission_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.csv")
