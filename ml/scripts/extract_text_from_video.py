...
def process_file(transcriber, file_path, output_folder_path):
...
def extract_all_texts_to_folder(transcriber, input_path, output_folder_path):
    os.makedirs(output_folder_path, exist_ok=True)

    if os.path.isfile(input_path):
        process_file(transcriber, input_path, output_folder_path)
        ...
        for idx, file_path in enumerate(file_paths, 1):
            ...
            process_file(transcriber, file_path, output_folder_path)
    else:
...
def main(args):
    ...
    extract_all_texts_to_folder(transcriber, args.input_path, args.texts_output_path)


if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='Extract text from audio and video files')
    parser.add_argument('--input_path', type=str, required=True, help='Path to the input file or directory containing audio and video files')
    parser.add_argument('--texts_output_path', type=str, required=True, help='Path to the output directory for transcribed texts')
